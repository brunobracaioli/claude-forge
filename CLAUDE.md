...
Claude Forge is a Claude Code skill/plugin that scaffolds a complete `.claude/` directory for any project. It generates CLAUDE.md, rules, skills (slash commands), agents, hooks, and settings from templates, with stack-specific presets.
...
bash scripts/bootstrap.sh <project_root> <stack>
# Stacks: flask-next | node | python | react | rust | generic | auto
```

## Architecture

The skill is invoked via `/claude-forge <stack>` or natural language. The workflow:
...
5. Settings merging requires `jq` — stack-specific permissions are merged into the base settings.json
...
templates/                 # Base templates copied to every project
  ...
  settings.json.template   # Base permissions (allow/deny)
  rules/                   # code-style, testing, security, git-workflow, agent-creation
  skills/                  # Slash commands as skills (canonical format)
    commit/SKILL.md        # /commit — conventional commit (disable-model-invocation)
    fix-issue/SKILL.md     # /fix-issue — GitHub issue fix (disable-model-invocation)
...
- New presets: create `presets/<name>/` with rules/architecture.md, CLAUDE.md.snippet, and optionally settings.json.override
